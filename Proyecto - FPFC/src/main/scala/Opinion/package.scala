...
package object Opinion {
  ...
  def confBiasUpdatePar(sb: SpecificBelief, swg: SpecificWeightedGraph): SpecificBelief = {
    val (fI, n) = swg
    // Divide el rango en dos mitades y paraleliza las tareas principales usando for
    val (left, right) = common.parallel(
      for {
        i <- 0 until sb.length / 2
        ai = (0 until sb.length).par.filter(fI(_, i) > 0.0).toList
        multiplicacion = for {
          j <- 0 until sb.length
        } yield {
          if (ai.contains(j)) {
      ...
      } yield sb(i) + (if (ai.nonEmpty) multiplicacion.sum / ai.size else 0.0),
      for {
        i <- sb.length / 2 until sb.length
        ai = (0 until sb.length).par.filter(fI(_, i) > 0.0).toList
        multiplicacion = for {
          j <- 0 until sb.length
        } yield {
